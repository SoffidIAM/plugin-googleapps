...
public class GmailSettingsService extends AppsForYourDomainService {
  ...
  public GmailSettingsService(
      String applicationName, String domain, String usernameMail, String password)
    ...
    this.setUserCredentials(usernameMail, password);
  }
...
//  }
   ...
   * Creates a send-as alias.
   *
   * @param codi d'usuari per crear el alias enviar-com.
   * @param name the name which e-mails sent using the alias are from.
   * @param address the e-mail address which e-mails sent using the alias are
   *        from.
   * @param replyTo (Optional) if set, this address will be included as the
   ...
   * @throws ServiceException if the insert request failed due to system error.
   */
  public void createSendAs(//Modificat per u88683
      String user, String name, String address, /*String replyTo,*/ boolean makeDefault)
      throws IllegalArgumentException, ServiceException, MalformedURLException, IOException {
...
//  }
   ...
   * @throws IOException If the insert request failed due to system error.
   */
	public void addEmailDelegate(String user, String delegationEmail) throws IllegalArgumentException, ServiceException,
		...
		//logger.log(Level.INFO, "Adding " + delegationEmail + " as an email delegate for user " + user + " ...");
   ...
   *         GData service.
   */
	public List<Map<String, String>> retrieveEmailDelegates(String user) throws IllegalArgumentException, IOException,
			ServiceException {
		if (isBlankOrNullString(user)) {
			throw new IllegalArgumentException();
		}

		// logger.log(Level.INFO, "Getting email delegation settings for user "
		// + user + " ...");
		...
		if (delegatesFeed != null) {
			...
			for (GenericEntry delegateEntry : delegateEntries) {
				...
				delegateMap.put(Constants.DELEGATION_ID, delegateEntry.getProperty(Constants.DELEGATION_ID));
   ...
   * @throws IOException If the insert request failed due to system error.
   */
	public void deleteEmailDelegate(String user, String delegationEmail) throws IllegalArgumentException, AppsForYourDomainException,
			ServiceException, IOException {
		if (isBlankOrNullString(user) || isBlankOrNullString(delegationEmail)) {
			throw new IllegalArgumentException();
		}

		//logger.log(Level.INFO, "Deleting the email delegate " + delegationEmail + " for user " + user + " ...");
		URL deleteUrl = new URL(Constants.PROTOCOL + "://" + Constants.APPS_APIS_DOMAIN + Constants.APPS_APIS_URL + "/" + domain + "/"
				+ user + "/" + Constants.DELEGATION + "/" + delegationEmail);
		delete(deleteUrl);
